...
public class Game extends Thread {
	...
	public boolean isInRange(Monster toBeChecked, Tower tower) {
		...
		int horizontalDiff = Math.max(horTower, horMonster) - Math.min(horTower, horMonster);
		int verticalDiff = Math.max(verTower, verMonster) - Math.min(horTower, horMonster);

		//Lasketaan tornin ja monsterin etäisyys janan pituuden kaavalla.
		distanceFromMonster = (int) Math.sqrt(Math.pow(horizontalDiff, 2) + Math.pow(verticalDiff, 2));
		if (distanceFromMonster <= tower.getRange()) {
			System.out.println("dist: " + distanceFromMonster + " tower.getRange()" + tower.getRange());
			return true;
		} else {
			return false;
		}
